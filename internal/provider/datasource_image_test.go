...
func TestImageToDataSourceModel(t *testing.T) {
	createdAt := time.Now()
	...
	image := &sdk.Image{
		...
		Spec: sdk.ImageSpec{
			...
			CpuArchitecture: sdk.ImageSpecCpuArchitectureArm64,
			Initializer:     sdk.ImageSpecInitializerNone,
			Boot:            sdk.ImageSpecBootBIOS,
		},
		Status: &sdk.ImageStatus{State: sdk.ResourceStateActive},
	}
	...
	assert.Equal(t, "arm64", model.CpuArchitecture.ValueString())
	assert.Equal(t, "none", model.Initializer.ValueString())
	assert.Equal(t, "BIOS", model.Boot.ValueString())
