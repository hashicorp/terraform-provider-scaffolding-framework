...
func TestInternetGatewayToResourceModel(t *testing.T) {
	createdAt := time.Now()
	...
	model, diags := internetGatewayToResourceModel(context.Background(), gtw)
	...
	assert.False(t, model.EgressOnly.ValueBool())
}

func TestInternetGatewayToResourceModel_EgressOnly(t *testing.T) {
	gtw := &sdk.InternetGateway{
		Metadata: &sdk.RegionalWorkspaceResourceMetadata{
			Name:      "igw-egress",
			Workspace: "workspace-1",
			Tenant:    "tenant-1",
			Region:    "region-1",
			Ref:       "seca.network/v1/tenants/tenant-1/workspaces/workspace-1/internet-gateways/igw-egress",
		},
		Spec: sdk.InternetGatewaySpec{
			EgressOnly: true,
		},
