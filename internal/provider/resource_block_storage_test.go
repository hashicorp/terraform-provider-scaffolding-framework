...
func TestBlockStorageToResourceModel(t *testing.T) {
	...
	block := &sdk.BlockStorage{
		...
		Spec: sdk.BlockStorageSpec{
			...
			SourceImageRef: &sdk.Reference{Resource: "images/image-1"},
		},
	}
	...
	assert.Equal(t, int64(100), model.SizeGB.ValueInt64())
	assert.Equal(t, "storage-skus/sku-1", model.SkuId.ValueString())
	assert.Equal(t, "images/image-1", model.SourceImageId.ValueString())
}
